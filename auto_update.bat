...
echo.
echo [1/8] Discovering and processing new videos only...
python channel_update.py
if errorlevel 1 goto :error

echo.
echo [2/8] Applying the public text quality gate...
python content_quality.py --rewrite-catalog
if errorlevel 1 goto :error

echo.
echo [3/8] Rewriting public video copy...
python rewrite_public_copy.py
if errorlevel 1 goto :error

echo.
echo [4/8] Updating the hash-based processing manifest...
python build_processing_manifest.py
if errorlevel 1 goto :error

echo.
echo [5/8] Verifying static deployment assets...
...
git add public/catalog.json public/search-index public/paragraph-index data/oka_video_summaries.json data/oka_youtube_map.json data/processing_manifest.json
