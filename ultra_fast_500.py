...
def run():
    ...
    print("=" * 80, flush=True)
    print(f"🚀 啟動極速 500 個複合關鍵字全量 Audit 審核 (總數: {len(combos)} 個)...", flush=True)
    print("=" * 80, flush=True)
    ...
    for idx, kw in enumerate(combos, 1):
        ...
        if c == 0:
            zero += 1
            bugs.append(f"「{kw}」無召回筆數")
        else:
            passed += 1
            if pure_eng:
                unmapped += 1
                bugs.append(f"「{kw}」發現待 Mapping 標題: {pure_eng[0]}")
    ...
    if bugs:
        print(f"\n🔍 排查出的潛在 Bug/漏網之魚 Sample:", flush=True)
        for b in bugs[:5]:
